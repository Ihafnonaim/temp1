...
class SimpleCNN(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim=10):
        ...
        self.projector = nn.Linear(hidden_dims[1], hidden_dims[1])
    ...
    def forward(self, x):
        ...
        x = self.relu(self.fc1(x))
        feature_emb = self.relu(self.fc2(x))
        proj_emb = self.projector(feature_emb)
        logits = self.fc3(feature_emb)

        # x = self.relu(self.fc2(x))
        # x = self.fc3(x)
        return proj_emb, feature_emb, logits
